...
@interface DepartureSeatReportVC () <UITableViewDelegate, UITableViewDataSource, UIActionSheetDelegate, MFMailComposeViewControllerDelegate, UIPrintInteractionControllerDelegate>
...
@implementation DepartureSeatReportVC
...
- (void)viewDidLoad
{
    ...
    self.title = @"Seat Departure Report";
    ...
    _lblDeparturedate.text = _dataToPass[@"tripDate"];
    _lblTripTime.text = _dataToPass[@"tripTime"];
    _lblBusNo.text = _dataToPass[@"busno"];

    _lblTripDate.hidden = YES;
    _lblTitleDate.hidden = YES;
...
- (void)convertViewToImage
{
    ...
    //    UIImage *tbImage = [self renderScrollViewToImage];
    ...
    //    UIImage *resizedtb = [UIImage imageWithCGImage:[tbImage CGImage] scale:(tbImage.scale * 2.0) orientation:(tbImage.imageOrientation)];
    ...
    //    _imageArray = @[resizedInfo, resizedHeader, resizedtb, resizedtotalView];
...
{
    //STARTING POINT
    NSString* cellid = @"seatdepartureCell";
    ...
    if (self.reachable) {
        ...
        cell.cellLblTransactionNo.text = dict[@"ticket_no"];
